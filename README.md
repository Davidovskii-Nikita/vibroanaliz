...
В наличии постоянное получение СКЗ ускорения по трем осям в функции *double Read_RawValue_AXEL(uint8_t deviceAddress, uint8_t regAddress)*. А также существуют цифровые фильтры:
...
~~~
Создается локальное имя в зоне **.lосаl** (mDNS), в фукнции *step* настраивается возможность таких OTA-обновлений, как отображение кнопок и вывод информации отладки в последовательный порт.
...
**В процессе прошивки** устройства через браузер цикл прошивки может повториться, что может привести к неправильной прошивке *ESP-01*. В этом случае нужно **дождаться** конца прошивки и перезагрузки модуля.
